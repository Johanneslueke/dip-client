...
func main() {
	var (
		...
		id      = flag.Int("id", 0, "DrucksacheText ID")
	)
	flag.Parse()
	...
	if *id == 0 {
		log.Fatal("DrucksacheText ID required (use -id flag)")
	}
	...
	drucksacheText, err := client.GetDrucksacheText(ctx, dipclient.ID(*id), nil)
	if err != nil {
		log.Fatalf("Failed to get drucksache text: %v", err)
	}

	output, err := json.MarshalIndent(drucksacheText, "", "  ")
	if err != nil {
